...
class Compiler {
	...
	public function compile(IReader $Reader): Generator {
		foreach ($Reader->read() as $index => $line){
			...
			if (!empty($line = rtrim($line))) {
				try {
					yield preg_replace_callback('/(\W|\A)@(' . Reglib::KEYWORD . ')\s*(.*)$/s', function ($Matches) {
						return $Matches[1] . (!empty($Matches[1]) ? ' ' : '') . $this->process(strtolower($Matches[2]), $this->parse($Matches[3])) . $Matches[3];
					}, $line);

				} catch (\Exception $Exception){
					throw new \Exception('Error in ' . (string)$File . ' on ' . $index . ': ' . $Exception->getMessage());
				}
			}
		}
	}
	 ...
	 * @throws \Exception
	 */
	protected final function parse(string &$source): string {
		if (empty($source)) {
			return $source;
		}
